...
export class FfmpegVideoService implements IVideoService {
  ...
  constructor(proxy?: Proxy) {
    this.proxy = proxy ?? new NullProxy();
  }
  ...
  async getVideoMetadata(filePath: string): Promise<VideoMetadata> {
    ...
    try {
      ...
      if (!videoMetaData)
        throw new Error("Не удалось получить мета-данные о видео-потоке");
      ...
      const error =
        err instanceof Error
          ? err
          : new Error("Неизвестная ошибка при получении мета-данных");
      logError(
        `Ошибка при получении мета-данных из HLS-потока: ${error.message}`,
      );
      throw error;
    }
  }

  async saveVideoToFolder(
    filePath: string,
    filename: string,
    folderPath: string,
  ): Promise<string> {
